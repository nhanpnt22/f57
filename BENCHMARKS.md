...
1. For raw performance, prefer a small BIT-LENGTH identifier (e.g. `ID57_LEN_32`) over a fixed-width one when globally distributed exact collision safety is not required. A 32-bit truncation fits into standard processor registers, avoiding heap-allocated `BigInt` operations almost entirely in low-level languages. Note this trade-off does NOT apply to fixed-width identifiers (`FIXED_2`..`FIXED_12`, the former "ID57-SHORT"): those are generated by first producing the full `ID57_LEN_128` identifier (a full 128-bit `BigInt` encode) and then slicing the resulting string to `k` characters, so they carry the same `BigInt` cost as `ID57_LEN_128` regardless of how short the output looks.
