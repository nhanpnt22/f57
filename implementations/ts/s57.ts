...
import {
  ...
  newInvalidVersionError,
  newKeyInvalidError,
  newKeyUnavailableError
} from './errors.js';
...
const S57_ALLOWED_LENGTHS: number[] = [S57Length.LEN_128, S57Length.LEN_256, S57Length.LEN_512];

function toBytes(input: ByteOrStringInput) {
...
function deriveKey(serverSecretKey: ByteOrStringInput, environmentSalt: ByteOrStringInput, context: string, length = 32) {
...
function validateLengthEnumForS57(lengthEnum: number, mappingResolver: (lengthEnum: number) => number, mappingValues: Map<number, number>) {
...
function truncateBits(rawBytes: Uint8Array, bitLength: number) {
...
function encodeR57_128(raw: Uint8Array) {
...
export class S57 {
  ...
  resolveKeys(): ResolvedKeys {
    return {
      ...
      b57_aes_256_key: deriveKey(
        this.serverSecretKey,
        this.environmentSalt,
        'S57/B57_AES_256/v1',
        32
      ),
      h57_key: deriveKey(
  ...
  hash(data: ByteOrStringInput, lengthEnum: number = H57Length.LEN_256) {
  ...
  id(data: ByteOrStringInput, lengthEnum: number = ID57Length.DEFAULT) {
    const input = toBytes(data);
  ...
  random_session(session_secret: ByteOrStringInput) {
  ...
  random_device(device_secret: ByteOrStringInput) {
  ...
  random_derived(master_secret: ByteOrStringInput, unique_input: ByteOrStringInput) {
  ...
  random_hybrid(...sources: ByteOrStringInput[]) {
    if (sources.length === 0) {
      throw newKeyInvalidError();
    }
  ...
  encrypt(plaintext: ByteOrStringInput, aad: ByteOrStringInput = new Uint8Array(0)) {
  ...
  decrypt(b57String: string, aad: ByteOrStringInput = new Uint8Array(0), expectedKeyId: number | undefined = undefined) {
