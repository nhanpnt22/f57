use b57::{
    ...
    id57_short_generate_default, r57_generate, HashFunction, H57Length, ID57Length, R57Mode,
};
...
fn e2e_roundtrip_and_profiles() {
    ...
    let h = h57_hash(input, HashFunction::Blake3, H57Length::Len128)
    ...
    let i57_h = i57_hash(input, HashFunction::Sha256, H57Length::Len128)
        .expect("i57 hash should succeed");
    assert!(!i57_h.is_empty());

    let i57_id_out =
        i57_id(input, Some(HashFunction::Blake3), ID57Length::Default).expect("i57 id should succeed");
