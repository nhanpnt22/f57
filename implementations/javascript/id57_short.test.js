...
test('id57-short deterministic output', () => {
  ...
  const a = id57ShortGenerate(input, ID57ShortLength.LEN_47);
  const b = id57ShortGenerate(input, ID57ShortLength.LEN_47);
  assert.equal(a, b);
});
...
test('id57-short supports only profile lengths', () => {
  ...
  for (const len of lengths) {
    const out = id57ShortGenerate(input, len);
    assert.equal(id57ShortIsValid(out), true);
    assert.equal(id57ShortIsCanonical(out), true);
    assert.equal(id57ShortVerify(input, out, len), true);
  }
});

test('id57-short rejects non-profile lengths', () => {
  assert.throws(
    () => id57ShortGenerate(te.encode('id57-short-invalid-len'), 128),
